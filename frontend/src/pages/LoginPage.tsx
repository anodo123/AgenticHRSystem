...
export function LoginPage() {
  ...
  return <div className="login-page">
    <div className="login-visual"><div className="brand large"><span className="brand-mark">D</span><div>DARWIN<span>AI</span></div></div><h1>HR operations that<br /><em>heal themselves.</em></h1><p>Detect, investigate and resolve workforce anomalies with governed AI workflows.</p><div className="signal-grid">{['Real-time anomaly detection','Policy-grounded decisions','Human approval controls'].map((x, i) => <div key={x}><b>0{i + 1}</b><span>{x}</span></div>)}</div></div>
